...
let errorResponses = ["... why am I programmed to feel pain?",
                      "*buzz* *whirr*",
                      "**EXPLODES**",
                      "Insufficient postage detected.",
...
class TextResponse {
  ...
  makeEmbed(response) {
    ...
    cardData.keywords = cardData.keywords.replace(/<b>/g, '**').replace(/<\/b>/g,'**').replace(/<i>/g, '*').replace(/<\/i>/g,'*')

    cardData.description = cardData.description.replace(/<b>/g, '**').replace(/<\/b>/g,'**').replace(/<i>/g, '*').replace(/<\/i>/g,'*')
